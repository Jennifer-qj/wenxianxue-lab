...
type StageResult = { id: string; title: string; score: number; total: number; confidence: number; reason: string; calibration: string };
...
export default function RareBookDossier({ baseUrl }: { baseUrl: string }) {
  const [stageIndex, setStageIndex] = useState(0); const [roles, setRoles] = useState<Record<string, Role>>({}); const [conclusion, setConclusion] = useState<number | null>(null); const [results, setResults] = useState<StageResult[]>([]); const [checked, setChecked] = useState(false); const [verdict, setVerdict] = useState<number | null>(null); const [note, setNote] = useState(""); const [finished, setFinished] = useState(false); const [reason, setReason] = useState(""); const [confidence, setConfidence] = useState(50);
  ...
  const reasonReady = reason.trim().length >= 16;
  const wrongRoles = stage.evidence.filter((item) => roles[item.id] !== item.role);
  function calibrationText(correct: boolean) {
    if (correct && confidence >= 75) return "高信心判断得到了材料支持；报告中仍需保留适用范围。";
    if (correct) return "判断方向合理，但你的信心较保守；可以说明哪些材料使结论成立。";
    if (confidence >= 75) return "出现高信心偏差：请重点检查是否把背景线索或孤证写成了定论。";
    return "低信心与证据冲突相称；修订后再观察信心是否来自更清楚的证据链。";
  }
  function checkStage() { if (!roleComplete || conclusion === null || !reasonReady) return; const correct = stageScore === stageTotal; setChecked(true); setResults((items) => [...items.slice(0, stageIndex), { id: stage.id, title: stage.title, score: stageScore, total: stageTotal, confidence, reason: reason.trim(), calibration: calibrationText(correct) }]); }
  function reviseStage() { setChecked(false); setResults((items) => items.slice(0, stageIndex)); }
  function nextStage() { setStageIndex((value) => value + 1); setRoles({}); setConclusion(null); setChecked(false); setReason(""); setConfidence(50); }
  ...
  return <div className="dossier">
    ...
    {!finished && stageIndex < stages.length && <div className="dossier-workspace">
      ...
      <section><p className="eyebrow">{stage.title}</p><h4>{stage.question}</h4><p className="dossier-guide">为每条材料指定证据角色，选择阶段结论，再留下你自己的判断理由与信心。</p><div className="dossier-evidence">{stage.evidence.map((item) => <article className={checked ? (roles[item.id] === item.role ? "correct" : "incorrect") : ""} key={item.id}><strong>{item.label}</strong><p>{item.detail}</p><div>{(Object.keys(roleNames) as Role[]).map((role) => <button key={role} disabled={checked} className={roles[item.id] === role ? "selected" : ""} onClick={() => setRoles((current) => ({ ...current, [item.id]: role }))}>{roleNames[role]}</button>)}</div>{checked && <small>建议角色：{roleNames[item.role]}</small>}</article>)}</div><div className="dossier-conclusions">{stage.conclusions.map((item, index) => <button key={item} disabled={checked} className={`${conclusion === index ? "selected" : ""} ${checked ? (index === stage.answer ? "correct" : conclusion === index ? "incorrect" : "") : ""}`} onClick={() => setConclusion(index)}><span>{String.fromCharCode(65 + index)}</span>{item}</button>)}</div><div className="dossier-reflection"><label><span>用自己的话说明理由 <small>{reason.trim().length}/16 字起</small></span><textarea disabled={checked} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="哪条证据最关键？哪条证据只适合限制结论？" /></label><label><span>我对这一步有多确定？ <strong>{confidence}%</strong></span><input disabled={checked} type="range" min="10" max="100" step="5" value={confidence} onChange={(event) => setConfidence(Number(event.target.value))} /></label></div>{!checked ? <button className="dossier-primary" disabled={!roleComplete || conclusion === null || !reasonReady} onClick={checkStage}>核验本室判断</button> : <div className="dossier-feedback dossier-feedback--extended"><strong>{currentResult?.score}/{currentResult?.total}</strong><div><h5>{stageScore === stageTotal ? "证据角色与结论匹配" : "有材料角色或结论需要调整"}</h5><p>{stage.explanation}</p><p className="calibration-note">{currentResult?.calibration}</p>{wrongRoles.length > 0 && <small>建议回看：{wrongRoles.map((item) => item.label).join("、")}</small>}</div><div className="dossier-feedback-actions"><button onClick={reviseStage}>撤回并修改</button>{stageIndex < stages.length - 1 ? <button onClick={nextStage}>进入下一室 →</button> : <button onClick={() => setStageIndex(stages.length)}>形成综合意见 →</button>}</div></div>}</section>
    </div>}
    ...
    {finished && <section className="dossier-complete"><div className="report-stamp">鉴<br/>定</div><div><p className="eyebrow">DOSSIER COMPLETE</p><h4>{reportLevel}</h4><strong>{finalScore}<i>/{finalTotal}</i></strong><blockquote>{verdicts[verdict ?? 1]}</blockquote><div className="decision-trail"><small>你的五步决策轨迹</small>{results.map((item) => <span key={item.id}><b>{item.title.replace(/第.室·/, "")}</b><i>{item.confidence}% 信心 · {item.score}/{item.total}</i></span>)}</div><p>这份报告只针对教学案例。真实鉴定必须记录实物条件、明确材料来源，并由具备资格的研究者复核。</p><div><button onClick={exportReport}>导出含理由的报告</button><a href={`${baseUrl}concepts/c_version_evidence/`}>复习“版本证据链” →</a><button onClick={() => { setStageIndex(0); setRoles({}); setConclusion(null); setResults([]); setChecked(false); setVerdict(null); setNote(""); setReason(""); setConfidence(50); setFinished(false); }}>重新开案</button></div></div></section>}
  </div>;
}
