...
function CaseReport({ lab, score, total, onRetry }: { lab: LabCase; score: number; total: number; onRetry?: () => void }) {
  const ratio = total ? score / total : 0; const level = ratio === 1 ? "证据链完整" : ratio >= .6 ? "基本成立，仍有缺口" : "需要重新检查";
  return <section className={`case-report ${ratio === 1 ? "excellent" : ratio >= .6 ? "developing" : "retry"}`} role="status"><div><small>LEARNING REPORT</small><strong>{score}<i>/{total}</i></strong></div><div><h4>{level}</h4><p>{engineAdvice[lab.engine]}</p><span>建议动作：打开关联概念，写下一条“这项材料不能直接证明什么”。</span></div>{onRetry && <button onClick={onRetry}>调整后再试</button>}</section>;
}

function ReasoningLab({ lab }: { lab: LabCase }) {
  const stages = (lab.config.stages ?? []).map((stage: any) => ({ id: idOf(stage), label: labelOf(stage) }));
  const [done, setDone] = useState<string[]>([]); const [submitted, setSubmitted] = useState(false); const complete = done.length === stages.length;
  return <div className="case-runner reasoning-runner"><p>逐步完成调查流程。每一步都代表证据链中不可省略的检查。</p><ol>{stages.map((stage: any, index: number) => <li className={done.includes(stage.id) ? "done" : ""} key={stage.id}><button disabled={submitted} onClick={() => setDone((items) => items.includes(stage.id) ? items.filter((id) => id !== stage.id) : [...items, stage.id])}><span>{done.includes(stage.id) ? "✓" : index + 1}</span><strong>{stage.label}</strong></button></li>)}</ol>{!submitted && <button className="case-submit" disabled={!complete} onClick={() => { saveCase(lab, stages.length, stages.length); setSubmitted(true); }}>形成有限度结论并归档</button>}{submitted && <CaseReport lab={lab} score={stages.length} total={stages.length} onRetry={() => { setDone([]); setSubmitted(false); }} />}</div>;
}
